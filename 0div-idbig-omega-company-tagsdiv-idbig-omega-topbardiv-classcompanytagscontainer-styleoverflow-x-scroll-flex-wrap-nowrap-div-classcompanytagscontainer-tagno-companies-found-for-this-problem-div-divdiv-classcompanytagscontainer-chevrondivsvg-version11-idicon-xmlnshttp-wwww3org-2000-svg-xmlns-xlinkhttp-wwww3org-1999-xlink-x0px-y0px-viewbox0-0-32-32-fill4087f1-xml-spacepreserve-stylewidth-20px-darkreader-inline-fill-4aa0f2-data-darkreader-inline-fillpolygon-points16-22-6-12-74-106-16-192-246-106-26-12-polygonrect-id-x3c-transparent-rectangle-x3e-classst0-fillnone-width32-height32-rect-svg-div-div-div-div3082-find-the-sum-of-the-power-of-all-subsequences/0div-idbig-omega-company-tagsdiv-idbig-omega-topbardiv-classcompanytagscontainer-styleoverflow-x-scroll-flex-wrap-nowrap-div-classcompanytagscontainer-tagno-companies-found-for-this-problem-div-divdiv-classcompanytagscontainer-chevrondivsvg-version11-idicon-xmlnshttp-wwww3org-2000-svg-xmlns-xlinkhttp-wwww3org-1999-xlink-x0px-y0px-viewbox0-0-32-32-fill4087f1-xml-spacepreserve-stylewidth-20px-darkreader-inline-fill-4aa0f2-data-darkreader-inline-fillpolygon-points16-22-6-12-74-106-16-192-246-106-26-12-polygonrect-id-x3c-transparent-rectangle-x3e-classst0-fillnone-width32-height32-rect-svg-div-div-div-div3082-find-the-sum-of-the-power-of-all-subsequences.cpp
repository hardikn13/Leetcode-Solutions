...
public:
    ...
    int mod = 1e9 + 7;
    int f(long long ind, long long sum, vector<int> &nums, int k, vector<vector<long long>> &dp)
    {
        int n = nums.size();
        
        if(sum == k)
            return power(2ll, (long long)n - ind);
        else if(sum > k)
            return 0;
        if(ind >= n)
            return 0;
        ...
        long long take = f(ind + 1, sum + nums[ind], nums, k, dp);
        long long notTake = 2 * f(ind + 1, sum, nums, k, dp);
